...
CREATE TABLE document_chunks (id integer PRIMARY KEY AUTOINCREMENT, document_id integer NOT NULL REFERENCES documents (id) ON DELETE CASCADE, chunk text NOT NULL, metadata text, embedding f32_blob (1024) NOT NULL);
...
CREATE TABLE configurations (name text PRIMARY KEY, value text, type text NOT NULL CHECK (type IN ('string', 'integer', 'boolean', 'blob', 'json')), description text NOT NULL);